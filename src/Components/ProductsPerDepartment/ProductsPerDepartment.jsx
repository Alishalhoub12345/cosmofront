...
function ProductsPerDepartment() {
  const { departmentLink } = useParams();
...
useEffect(() => {
  ...
  //     `https://www.cosmo.global/laravel/api/getdepartment/${departmentLink}`,
  //     {
  //       params: { locale: selectedLang },
  //     }
  //   );

  //   const departmentData = departmentRes.data.message;
  //   return departmentData;
  // };

  // const { data: departmentData } = useQuery(
  //   ["products", departmentLink],
  //   getProductByDepartment
  // );

  const getProductsPerDepartment = async () => {
    let response;
    try {
      response = await axios.get(
        `https://www.cosmo.global/laravel/api/get-department-with-products/${departmentLink}/${page}?productVisible=1`,
        {
          params: { locale: selectedLang },
        }
      );
      ...
      if (page === 1) {
        setDepartmentProducts(response.data.data);
      } else {
        setDepartmentProducts((prevProducts) => [
          ...prevProducts,
          ...response.data.data,
        ]);
      }
        ...
        >
          ...
          {departmentData && (
            <p className="sm:text-[12px] px-[2%] py-[0.5%]">
              <strong>{isArabic ? departmentData.name_ar : departmentData.name}</strong>
            </p>
          )}
        </div>
      </div>
      ...
      {loading ? (
        <div className="">
          <div className="min-h-[100vh] xl:min-h-[80vh]  h-[auto] flex-wrap flex justify-evenly px-[7%] py-[3%]  items-start gap-[0.1%] flex-row font-[100] text-white font-[FahKwang]">
            {departmentProducts?.map((singleProdPerDep, index) => (
              <div
                key={index}
                className="relative bg-[#eeeeee] w-[20%]  md:w-[40%] mt-[0.1%] md:mt-[0.5%] "
              >
                {singleProdPerDep.productSale && (
...
>
                      ...
                      <div className="min-h-[20rem] xl:min-h-[5rem] lg:min-h-[10rem] sm:min-h-[5rem] mt-[0.1%] md:mt-[0.5%] ">
                        <img
                          ...
                          src={`https://www.cosmo.global/laravel/api/storage/${singleProdPerDep.media1}`}
                          alt={`${singleProdPerDep.id}`}
                          onError={(e) => {
                    ...
                    {!singleProdPerDep.totalQuantity &&
                      singleProdPerDep.totalQuantity <= 0 && (
                        <>
                      ...
                      {singleProdPerDep.productSale ? (
                        ...
                        >
                          <p className=" font-[FrutigerLTCom-Roman] xl:text-[10px]">
                            {(
                              (singleProdPerDep.productPrice -
                                singleProdPerDep.productPrice *
                                  (singleProdPerDep.productSale / 100)) *
                              currencyValue
                            ).toFixed(2)}
                            {currencyUsed}
                          </p>
                          <p className=" text-[12px] xl:text-[10px]  font-[FrutigerLTCom-Roman] line-through text-[#a9a9a9]">
                            {(
                              singleProdPerDep.productPrice * currencyValue
                            ).toFixed(2)}
                            {currencyUsed}
                          </p>
                        </div>
