...
function ProductsPerSubcategory() {
  ...
  const getProductBySubcategory = async () => {
    const subcategoryRes = await axios.get(
      `http://localhost:8000/api/SubcategoryCorrespondingDepandCat/${subcategoryLink}`,
      {
  ...
  const getProductsPerSubcategory = async () => {
    ...
    try {
      response = await axios.get(
        `http://localhost:8000/api/product/${subcategoryLink}/${page}?productVisible=1`,
        {
          params: { locale: selectedLang },
        }
      );
        ...
        {subcategoryData?.category?.map((cat) => (
          ...
          >
            ...
            <Link
              ...
              className="border-x-[1.5px] sm:text-[12px] border-x-[black] px-[2%] py-[0.5%] hover:cursor-pointer"
            >
              {cat.categoryName}
            </Link>
            {cat?.departments.map((dep) => (
              <Link
                ...
                key={dep.id}
                className="sm:text-[12px] px-[2%] py-[0.5%] cursor-pointer"
              >
      ...
      {loading ? (
        <div className="">
          <div className="min-h-[100vh] xl:min-h-[80vh]  h-[auto] flex-wrap flex justify-evenly px-[7%] py-[3%]  items-start gap-[0.1%] flex-row font-[100] text-white font-[FahKwang]">
            {subcategoryProducts?.map((singleProdSub, index) => (
              <div
                key={index}
                className="relative bg-[#eeeeee] w-[20%]  md:w-[40%] mt-[0.1%] md:mt-[0.5%] "
              >
                {singleProdSub.productSale && (
                ...
                <>
                  <div className=" flex flex-col justify-between min-h-[20rem] xl:min-h-[5rem] lg:min-h-[10rem] my-[1%]">
                    <Link
                      to={`/single/product/${singleProdSub.productLink}/${singleProdSub.productSKU}`}
                    >
                      <div className="min-h-[20rem] xl:min-h-[5rem] lg:min-h-[10rem] sm:min-h-[5rem] mt-[0.1%] md:mt-[0.5%] ">
                        <img
                          className=" w-[100%] h-[100%] object-cover "
                          src={`http://localhost:8000/api/storage/${singleProdSub.media1}`}
                    ...
                    {!singleProdSub.totalQuantity &&
                      singleProdSub.totalQuantity <= 0 && (
                        <>
                      ...
                      {singleProdSub.productSale ? (
                        ...
                        >
                          <p className=" font-[FrutigerLTCom-Roman] xl:text-[10px]">
                            {(
                              (singleProdSub.productPrice -
                                singleProdSub.productPrice *
                                  (singleProdSub.productSale / 100)) *
                              currencyValue
                            ).toFixed(2)}
                            {currencyUsed}
                          </p>
                          <p className=" text-[12px] xl:text-[10px]  font-[FrutigerLTCom-Roman] line-through text-[#a9a9a9]">
                            {(
                              singleProdSub.productPrice * currencyValue
                            ).toFixed(2)}
                            {currencyUsed}
                          </p>
                        </div>
                        ...
                        >
                          {(singleProdSub.productPrice * currencyValue).toFixed(
                            2
                          )}
                          {currencyUsed}
                        </p>
