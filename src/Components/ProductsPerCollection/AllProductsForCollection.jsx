...
function AllProductsForCollection() {
  ...
  //     `http://localhost:8000/api/SubcategoryCorrespondingDepandCat/${subcategoryId}`, {
  ...
  const getAllProductsForCollection = async () => {
    ...
    try {
      response = await axios.get(
        `http://localhost:8000/api/all-products-collection/${page}?productVisible=1`,
        {
          params: { locale: selectedLang },
        }
      );
                    ...
                    >
                      <div className="min-h-[20rem] xl:min-h-[5rem] lg:min-h-[10rem] sm:min-h-[5rem] mt-[0.1%] md:mt-[0.5%] ">
                        <img
                          ...
                          src={`http://localhost:8000/api/storage/${AllProdColl.media1}`}
                          alt={`${AllProdColl.id}`}
                          onError={(e) => {
