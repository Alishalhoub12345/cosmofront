...
function CartComponent() {
  ...
  const getProductsInCart = async () => {
    ...
    if (cart.length > 0) {
      ...
      const res = await axios.get(
        "http://localhost:8000/api/product-size-cart",
        {
          params: {
            sizeIds: sizeIds,
            locale: selectedLang,
    ...
    if (updatedCart.length > 0 && selectedCountry !== 109) {
      ...
      if (selectedCountryObj) {
        const response = await axios.get(
          `http://localhost:8000/api/shipping-details/${selectedCountryObj.id}?totalWeight=${totalWeight}`
        );
  ...
  const updateItemStatus = async (products) => {
    try {
      await axios.post(
        `http://localhost:8000/api/update-item-status/${cartRefNumber}`,
        {
  ...
  const updateCartInDatabase = async (updatedCart) => {
    if (cartRefNumber) {
      ...
      const res = await axios.post(
        `http://localhost:8000/api/edit-cart/${cartRefNumber}`,
  ...
  const getUpdatedShippingPrice = async () => {
    ...
    if (selectedCountryObj && cart.length > 0) {
      const response = await axios.get(
        `http://localhost:8000/api/shipping-details/${selectedCountryObj.id}?totalWeight=${prodWeight}`
      );
      const price = response.data.shippingFee;
      setCountryPrice(price);
    }
  ...
  const addCartToDbForGuest = async () => {
    ...
    if (cartRefNumber) {
      const response = await axios.get(
        `http://localhost:8000/api/single-cart/${cartRefNumber}`
      );
                          ...
                          >
                            ...
                            >
                              <div className="h-[150px] flex justify-center items-center sm:w-[60px] sm:h-[60px] relative  w-[130px] xl:w-[120px]">
                                ...
                                <img
                                  src={`http://localhost:8000/api/storage/${prod.product.media1}`}
